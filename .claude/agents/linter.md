...
- DO: name script files as `{function}-{domain}.sh` matching their parent skill directory (e.g., `extract-figma/extract-figma.sh`). DON'T: use bare verb like `extract.sh` or non-shell extensions like `.py`. WHY: script name mirrors skill dir — one pattern searches both; `.sh` wrappers handle venv and deps uniformly.
...
- DO: verify every agent file contains a sub-step whose DO begins with "Re-read" in its Rules or Steps section. DON'T: assume short agents don't need it or that the AI will remember on its own. WHY: agents run long workflows — without a re-read sub-step, AI forgets constraints by the last step.
...
2. Glob for `.md` under `.claude/agents/` and `plugins/*/agents/`, `SKILL.md` under `plugins/*/skills/*/`, and `.sh` under `plugins/*/skills/*/` when no paths are provided. DON'T: assume files live in only one directory. WHY: agents can live in global or plugin directories — missing a location means missing violations.
