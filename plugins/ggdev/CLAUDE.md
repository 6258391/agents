...
### N1. Command file naming

**DO**: verify command files use `{domain}.md` pattern. e.g., `frontend.md`, `wordpress.md`, `backend.md`.

**DON'T**: accept command files with function suffixes or compound names. e.g., `frontend-build.md`, `wp-theme.md`.
**WHY DON'T**: commands are entry points per domain. One domain = one command file.
...
**DO**: verify agent files are in `skills/team-management/references/`.

**DON'T**: accept agent files in `agents/` directory.
**WHY DON'T**: `subagent_type` does NOT load agent definitions for team members (confirmed by testing). Files in `agents/` are never read at runtime.
...
### F2. Command frontmatter — required fields

**DO**: verify commands have exactly these required fields: `description`, `argument-hint`.

**DON'T**: accept commands missing `argument-hint` or having extra unknown fields.
**WHY DON'T**: `argument-hint` tells the user what flags are available. Missing it makes the command undiscoverable.
...
## Sections — Commands and Agents

### S1. Use XML tags

**DO**: verify commands and agents use XML tags (`<scope>`, `<constraints>`, `<workflow>`, `<self-check>`).

**DON'T**: accept markdown headers (`## Scope`, `## Constraints`) in commands or agents.
**WHY DON'T**: XML tags are the convention for commands/agents. Markdown headers are for skills only. Mixing the two confuses file type.
...
**DO**: verify identity is a unique role statement specific to this agent/command.
...
**WHY DON'T**: skills have a different pattern from commands/agents. XML tags in skills signal the author confused the two patterns.
...
### D1. Command IS the Lead

**DO**: verify no separate Lead agent file exists. The command file itself serves as Lead.

**DON'T**: accept separate agent files that coordinate other agents. That role belongs to the command.
**WHY DON'T**: command runs in main session with full tool access. Separate Lead agent adds indirection without value.
...
Exception: commands and agents are self-contained — they don't have access to CLAUDE.md at runtime. Critical rules MAY be repeated in commands/agents when needed for correct behavior.

### D9. TeamCreate first, then TaskCreate

**DO**: verify commands create team before creating tasks. Order: TeamCreate → TaskCreate.

**DON'T**: accept commands that create tasks before TeamCreate.
...
**DO**: verify task descriptions in commands contain only file paths.
...
### D12. Command defines workspace — skills/agents receive paths

**DO**: verify paths are defined in commands and passed to skills/agents via invoke prompts or task descriptions.

**DON'T**: accept hardcoded directory paths in skills or agents. e.g., `Read from workspace/specs/`.
**WHY DON'T**: different domains have different directory structures. Hardcoding breaks reuse across commands.

### D13. Lead speaks two languages

**DO**: verify commands use structured/technical language with agents and human-preferred language with user.

**DON'T**: accept commands using the same language for both audiences.
**WHY DON'T**: agents need exact data to process. Humans need visual context to answer accurately.

### D14. Lead coordinates only

**DO**: verify commands delegate analysis to agents, never analyze input themselves.

**DON'T**: accept commands that perform their own analysis before delegating.
...
**DO**: verify commands use SendMessage to each agent by name.
...
**DO**: verify commands follow exact order: TeamCreate → TaskCreate → Agent (spawn members) → TaskUpdate (assign owner).
