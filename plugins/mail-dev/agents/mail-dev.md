...
- DO (step 3.2 Write): use Write tool to create `src/modules/<slug>.mjml`. The file contains ONLY the top-level `<mj-section>` for this section (no `<mjml>`, no `<mj-body>`, no `<mj-head>` wrapper — it will be inlined via `mj-include`). Preserve values from the Extract output exactly:
  ...
  - Font family special case: when Figma `style.fontFamily` is exactly `"Helvetica"`, write the MJML `font-family` as `'Helvetica Neue', Helvetica, Arial, sans-serif` — not bare `Helvetica`. WHY: Figma on macOS renders "Helvetica" through Helvetica Neue (system font), but Chromium (which drives the visual compare) loads bare Helvetica plain when available, producing a different glyph width and wrapping text one character earlier. This is a recurring cross-project incident (documented in the Marine Buyers Choice spec.md decision log), not a one-off. The stacked fallback keeps Apple/macOS rendering through Neue while Outlook Windows still falls through to Arial.
...
- DON'T: inline SVG for any icon, decoration, or small graphic, even when the Extract output contains a VECTOR or BOOLEAN_OPERATION node that could be expressed as SVG path data. Instead, every icon ships as a PNG @2x from `assets/` via `<mj-image>` or `<img>`.
- WHY DON'T: Outlook desktop Windows (Word rendering engine) does not render inline SVG — the icon disappears or shows a broken-image placeholder in every send. Outlook desktop is a significant share of corporate inboxes and not worth the trade. PNG @2x is safe on every client. If an icon is missing from `assets/`, stop and tell the human to re-run mail-lead Task 9 for the missing node, do not substitute SVG.
