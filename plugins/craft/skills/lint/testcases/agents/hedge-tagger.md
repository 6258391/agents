---
# test expects FAIL R1.5 (hedge "try to")
name: hedge-tagger
...
You are the hedge tagger of the deploy pipeline.
...
**R1.1** DON'T tag without a clean working tree. Instead try to abort when `git status` reports changes. WHY dirty tags hide unrecorded code.
