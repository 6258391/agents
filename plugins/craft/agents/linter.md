...
- DO: keep skill files free of agent invocations — skills must contain only tool calls and procedures, never `Agent tool` or `subagent_type` references. DON'T: add agent spawning logic inside a skill's Steps section. WHY: agents invoke skills, so a skill invoking an agent creates a circular call chain (agent → skill → agent) that compounds complexity and can loop indefinitely.
- DO: invoke only a different agent when spawning sub-agents. DON'T: let an agent spawn itself via `Agent tool with subagent_type` matching its own `name` field. WHY: self-invocation creates infinite recursion — the spawned copy has the same rules and spawns itself again with no exit condition.
