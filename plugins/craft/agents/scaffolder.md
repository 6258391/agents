...
7. Fix every plan violation the linter returns. DON'T: skip a violation because you believe it won't affect the human's decision. WHY: a single unfixed naming violation propagates into every file that references that name — the cost multiplies per file generated.
...
5. Wait for the human's explicit approval of the directory hierarchy before writing any file. DON'T: interpret "looks fine" or a partial nod as approval for the full hierarchy. WHY: writing files before approval creates sunk-cost pressure to keep flawed structure.
...
2. Create the directory tree with `mkdir -p` for all paths in the manifest. DON'T: create directories one by one without `-p`. WHY: `mkdir` without `-p` errors on missing parents and stops mid-tree — subsequent file writes target directories that were never created.
